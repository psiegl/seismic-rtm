...
#if 0
            s_sum = _mm_add_ps( _mm_fmsub_ps( s_two,
                                                s_actual,
                                            s_ppf_aligned ),
...
#else
            s_sum = _mm_add_ps( _mm_add_ps( s_above1, s_under1),
                                 _mm_add_ps( s_left1, s_right1));

            s_above2 = _mm_add_ps( _mm_add_ps( s_right2, s_left2),
                                   _mm_add_ps( s_under2, s_above2));

            s_sum = _mm_fmsub_ps( s_sixteen, s_sum,  s_above2);
            s_sum = _mm_fnmadd_ps( s_sixty, s_actual, s_sum );
            s_sum = _mm_fmadd_ps( s_vel_aligned, s_sum, _mm_fmsub_ps(s_two, s_actual, s_ppf_aligned) );
#endif
...
#define SYM_KERNEL_CAP { .fma3 = 1, .sse = 1 }
